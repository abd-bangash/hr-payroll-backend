...
const createEmployee = async (req, res) => {
  try {
    ...
    await logAudit(
      ...
      'Employee',
      employee._id,
      { employeeId: employee.employeeId, userId: employeeData.userId },
      req.ip,
      req.get('User-Agent')
    );
...
const getEmployees = async (req, res) => {
  try {
    ...
    const employees = await Employee.find(filter)
      ...
      .populate('employmentDetails.reportingManager', 'personalInfo.firstName personalInfo.lastName employeeId')
      .limit(limit * 1)
      .skip((page - 1) * limit)
      .sort({ createdAt: -1 });
